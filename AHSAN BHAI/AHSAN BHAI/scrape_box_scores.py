...
import argparse
import threading
import requests  # exception types only; MaxPreps fetches use curl_cffi / system curl
...
# Keep JSON/XHR header set minimal for /_next/data/… endpoints.
HEADERS = {
    "User-Agent": (
...
# Top-level page navigation (schedule, box score). Browsers and curl use
# Accept */* with no Referer; the JSON Accept set below is for /_next/data only.
HTML_HEADERS = {
...
# Chrome TLS impersonation profile — must stay aligned with HEADERS User-Agent.
CURL_IMPERSONATE = "chrome124"

# ── HTTP transport (curl_cffi primary, system curl fallback) ─────────────────
# Plain Python `requests` is NOT used for MaxPreps: its TLS fingerprint is
# rejected with 406 even from US IPs. curl_cffi impersonates Chrome in-process;
# system curl is the backup if curl_cffi is missing or a single fetch fails.
...
def _http_backend_label():
    if _CFFI_AVAILABLE:
        return f"curl_cffi ({CURL_IMPERSONATE}) + system curl fallback"
    return "system curl only (pip install curl_cffi recommended)"


def _get_cffi_session(kind="html"):
    """Thread-local curl_cffi Session; kind is 'html' or 'json'."""
    attr = f"cffi_session_{kind}"
    if not hasattr(_tls, attr):
        if not _CFFI_AVAILABLE:
            setattr(_tls, attr, None)
        else:
            base_headers = HTML_HEADERS if kind == "html" else HEADERS
...
def _fetch_via_curl(url, timeout=30, extra_headers=None):
    """Fallback fetch using system curl. Returns (status_code, body, final_url)."""
    import shutil
    import subprocess
    ...
    marker = "\n__CURL_META__"
    hdrs = dict(HTML_HEADERS)
    if extra_headers:
        hdrs.update(extra_headers)
    cmd = [
        curl_bin, "-sL",
        "-A", hdrs.get("User-Agent", HTML_HEADERS["User-Agent"]),
        "-H", f"Accept: {hdrs.get('Accept', '*/*')}",
        "-H", f"Accept-Language: {hdrs.get('Accept-Language', HTML_HEADERS['Accept-Language'])}",
    ]
    ...
    cmd.extend([
        "-w", marker + "%{http_code}__URL__%{url_effective}",
...
def _fetch_via_cffi(url, timeout=25, allow_redirects=True, session_kind="html",
                    extra_headers=None):
    """In-process Chrome-impersonating GET. Returns (status, text, final_url)."""
    session = _get_cffi_session(session_kind)
    if session is None:
        return None, None, None
    try:
        r = session.get(
...
def _http_get(url, timeout=25, allow_redirects=True, session_kind="html",
              extra_headers=None):
    """GET a MaxPreps URL. curl_cffi first, then system curl. No plain requests."""
    if _CFFI_AVAILABLE:
        try:
            status, text, final_url = _fetch_via_cffi(
                url, timeout=timeout, allow_redirects=allow_redirects,
                session_kind=session_kind, extra_headers=extra_headers,
            )
            if status == 200 and text:
                return 200, text, final_url or url
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
            raise

    merge_headers = dict(HTML_HEADERS if session_kind == "html" else HEADERS)
    if extra_headers:
        merge_headers.update(extra_headers)
    code, body, final_url = _fetch_via_curl(
        url, timeout=timeout, extra_headers=merge_headers,
    )
    if code == 200 and body:
        return 200, body, final_url or url
    return code or 406, None, final_url or url


def _http_get_page(url, timeout=25, allow_redirects=True):
    """GET a MaxPreps HTML page. Returns (status_code, text, final_url)."""
    return _http_get(
        url, timeout=timeout, allow_redirects=allow_redirects, session_kind="html",
    )


def _is_geo_block(status=None, body=None):
    """True when MaxPreps serves its geographic restriction page."""
    if status == 403:
        return True
    text = body or ""
    return "Geo-block" in text or "403 - Geo-block" in text


def _geo_block_error_message():
    return (
        "MaxPreps returned 403 Geo-block — your IP appears to be outside an "
        "allowed region (common outside the US).\n"
        "  Fix: connect a US VPN, confirm https://www.maxpreps.com loads in "
        "your browser, then re-run the scraper.\n"
        "  Note: this is separate from the 406 bot-detection issue; no header "
        "or TLS change bypasses a geo-block."
    )


def _check_maxpreps_access():
    """Probe MaxPreps once. Returns an error message if geo-blocked, else None."""
    url = (
        "https://www.maxpreps.com/nm/albuquerque/la-cueva-bears/"
        "basketball/25-26/schedule/"
    )
    code, body, _ = _fetch_via_curl(url, timeout=20)
    if _is_geo_block(code, body):
        return _geo_block_error_message()
    return None
...
def _schedule_html_url(team_path, season_suffix=None):
    """Canonical schedule page URL (the one a browser loads)."""
    if season_suffix:
...
def _contests_from_next_data(html):
    """Parse the contests array embedded in __NEXT_DATA__ on a schedule page.

    As of the 2026 redesign MaxPreps often returns 406 for the direct
    /_next/data/{bid}/…/schedule.json endpoint even when the HTML schedule
    page loads fine — verified against NM La Cueva 25-26. This path is the
    reliable fallback (and is now tried automatically by fetch_schedule()).
    """
    m = re.search(
    ...
    return (
        pp.get("initialPageProps", {}).get("contests")
        or pp.get("contests")
    )


def _fetch_contests_from_schedule_html(team_path, season_suffix=None):
    """Load a team's schedule HTML page and return its contests list."""
    url = _schedule_html_url(team_path, season_suffix)
    time.sleep(DELAY)
    try:
        status, text, _final = _http_get_page(url, timeout=25)
        if status == 404:
            return {"_expired": True}
        if status != 200 or not text:
            print(f"    [WARN] schedule HTML {status} for {team_path}")
            return None
        contests = _contests_from_next_data(text)
        if contests is not None:
            return contests
        print(f"    [WARN] no __NEXT_DATA__ contests on schedule page for {team_path}")
        return None
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
        raise
    except Exception as e:
        print(f"    [WARN] schedule HTML fetch failed for {team_path}: {e}")
        return None
...
def _game_entries_from_schedule_html(html):
    """Extract (game_url, contest_guid, ssid) from schedule page anchor tags.

    Used when MaxPreps streams an empty __NEXT_DATA__ shell but still renders
    the schedule table with Box Score links in HTML (common on 2026 pages).
    ssid is usually unavailable here; scrape_game() falls back to game_url.
    """
    entries = []
    seen = set()
...
def fetch_game_entries(build_id, team_path, season_suffix=None):
    """Return [(game_url, contest_guid, ssid), …] for a team's schedule.

    Tries, in order:
      1. HTML schedule page (__NEXT_DATA__ contests array)
      2. HTML schedule page (Box Score / game anchor links)
      3. Legacy /_next/data/…/schedule.json endpoint

    Returns {"_expired": True} on a confirmed 404 season page, or None when
    every source failed (network/geo block).
    """
    # ── 1 & 2: rendered schedule HTML (works when schedule.json returns 406) ──
    url = _schedule_html_url(team_path, season_suffix)
    time.sleep(DELAY)
    try:
        status, text, _final = _http_get_page(url, timeout=25)
        ...
        if status == 200 and text:
            ...
            if contests:
                ...
                if entries:
                    return entries
            link_entries = _game_entries_from_schedule_html(text)
            if link_entries:
                return link_entries
            # Page loaded but no games listed for this season.
            return []
        print(f"    [WARN] schedule HTML {status} for {team_path}")
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
        raise
    ...
    # ── 3: legacy JSON endpoint (often 406 — try requests then curl) ────────
    if season_suffix:
        json_url = (
            f"https://www.maxpreps.com/_next/data/{build_id}/"
    ...
    try:
        json_status, json_text, _ = _http_get(
            json_url, timeout=25, session_kind="json", extra_headers=json_headers,
        )
        if json_status == 404:
            return {"_expired": True}
        if json_status == 200 and json_text:
            ...
            return get_game_entries(contests) if contests else []
        if json_status and json_status != 200:
            print(f"    [WARN] schedule.json {json_status} for {team_path}")
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
        raise
    except Exception as e:
        print(f"    [WARN] schedule.json fetch failed for {team_path}: {e}")
...
def _fetch_build_id_raw():
    ...
    seed_pages = [
        # Season-suffixed pages match the URLs we scrape (25-26 etc.).
        ...
        "https://www.maxpreps.com/ca/concord/de-la-salle-spartans/basketball/25-26/schedule/",
    ...
    for attempt, wait in enumerate(delays, 1):
        for url in seed_pages:
            try:
                ...
                if status == 403:
                    _, probe_body, _ = _fetch_via_curl(url, timeout=30)
                    if _is_geo_block(403, probe_body):
                        saw_geo_block = True
                    continue
                if status != 200 or not text:
                    continue
...
def fetch_schedule(build_id, team_path, season_suffix=None):
    ...
    Thin wrapper kept for callers that still want the raw contests array.
    Prefer fetch_game_entries() for new code.
    """
    result = fetch_game_entries(build_id, team_path, season_suffix)
    if isinstance(result, dict):
        return result
    if result is None:
        return None
    # Re-fetch via HTML/JSON internals is wasteful; callers using fetch_schedule
    # only need the contests shape — rebuild a minimal stub when we only have links.
    if result == []:
        return []
    # If we got link tuples back, fetch_schedule can't represent them — signal
    # failure so legacy callers retry; _scrape_team uses fetch_game_entries().
    if result and isinstance(result[0], tuple):
        html = _fetch_schedule_html_text(team_path, season_suffix)
        if html:
            contests = _contests_from_next_data(html)
            if contests is not None:
                return contests
        return None
    return None
...
def _fetch_schedule_html_text(team_path, season_suffix=None):
    """Best-effort schedule HTML text (no parsing)."""
    url = _schedule_html_url(team_path, season_suffix)
    try:
        status, text, _final = _http_get_page(url, timeout=25)
...
def scrape_game(game_url, guid, ssid, our_team_name, team_id, opp_index=None):
    ...
    try:
        ...
        if stats_url != final_url:
            time.sleep(DELAY)
            st2, html2, final_url2 = _http_get_page(stats_url, timeout=25, allow_redirects=True)
...
def _scrape_team(team, season_suffix=None, opp_index=None):
    ...
    entries = game_entries if isinstance(game_entries, list) else []
    team_games = []
    for game_url, guid, ssid in entries:
        if not game_url:
            continue
        if not guid:
            m = re.search(r"[?&]c=([A-Za-z0-9_-]+)", game_url or "")
            if m:
                guid = decode_contest_guid(m.group(1))
...
def run(input_file=None, output_file=None, sport="boys", season="2025-2026", workers=None,
        run_accumulator=True, limit=None):
    """
    ...
    input_team_paths = {team_url_to_path(t["teamUrl"]) for t in teams}
    ...
    print(f"HTTP client      : {_http_backend_label()}")
    if limit is not None and limit > 0:
        print(f"Limit            : first {limit} teams (--limit test mode)")
    print(f"Workers          : {workers}\n")

    geo_err = _check_maxpreps_access()
    if geo_err:
        print(f"[ERROR] {geo_err}")
        raise RuntimeError(geo_err)
    ...
    if os.path.exists(output_file):
        try:
            with open(output_file, "r", encoding="utf-8") as f:
                ...
                # Only resume state for teams in THIS input file.
                processed_teams &= input_team_paths
                errors = [
                    e for e in errors
                    if team_url_to_path(e.get("teamUrl", "")) in input_team_paths
                ]
                all_games = [
                    g for g in all_games
                    if (g.get("team") or {}).get("team_id") in input_team_paths
                ]
                # Re-scrape teams that were marked done but produced zero games
                # (common when an earlier run hit a bug mid-pipeline).
                for tid in list(processed_teams):
                    n = sum(
                        1 for g in all_games
                        if (g.get("team") or {}).get("team_id") == tid
                    )
                    if n == 0:
                        processed_teams.discard(tid)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Process only the first N teams from the input file (for testing). "
             "Resume still works: re-run without --limit to continue remaining teams.",
    )
